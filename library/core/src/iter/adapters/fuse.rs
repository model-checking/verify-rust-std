...
mod verify {
    ...
    fn any_fuse_iter<'a, T>(orig_slice: &'a [T]) -> Fuse<crate::slice::Iter<'a, T>> {
        Fuse::new(any_slice(orig_slice).iter())
    }
    ...
    macro_rules! check_get_unchecked {
        ($harness:ident, $elem_ty:ty, $max_len:expr) => {
            ...
            fn $harness() {
                ...
                let mut it = any_fuse_iter::<$elem_ty>(&array);
                let idx = kani::any_where(|i: &usize| *i < it.iter.as_ref().unwrap().size_hint().0);
                let _ = unsafe { it.__iterator_get_unchecked(idx) };
            }
        };
    }
    check_get_unchecked!(check_fuse_get_unchecked_unit, (), isize::MAX as usize);
    check_get_unchecked!(check_fuse_get_unchecked_u8, u8, u32::MAX as usize);
    check_get_unchecked!(check_fuse_get_unchecked_char, char, 50);
    check_get_unchecked!(check_fuse_get_unchecked_tup, (char, u8), 50);
}
