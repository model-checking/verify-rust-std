...
class Flt2decHarnessesTests(unittest.TestCase):
    ...
    def test_complete_disjoint_batches_on_both_platforms(self):
        ...
        self.assertEqual(len(self.groups), 57)
        self.assertEqual(len({group["name"] for group in self.groups}), 57)
        self.assertEqual(len(self.dedicated), 221)
        self.assertEqual(len(set(self.dedicated)), 221)
