...
class Flt2decHarnessesTests(unittest.TestCase):
    ...
    def test_complete_disjoint_batches_on_both_platforms(self):
        ...
        self.assertEqual(len(self.groups), 59)
        self.assertEqual(len({group["name"] for group in self.groups}), 59)
        self.assertEqual(len(self.dedicated), 223)
        self.assertEqual(len(set(self.dedicated)), 223)
